...
/// 对原生的UITabBarItemPositioning进行扩展，通过UITabBarItemPositioning设置时，系统会自动添加insets，这使得添加背景样式的需求变得不可能实现。SPTabBarItemPositioning完全支持原有的item Position 类型，除此之外还支持完全fill模式。
///
...
public enum SPTabBarItemPositioning: Int {
    case automatic
    case fill
    case centered
...
internal protocol SPTabBarDelegate: NSObjectProtocol {
...
open class SPTabBar: UITabBar {
    /// 代理
    internal weak var customDelegate: SPTabBarDelegate?
    ...
    /// 是否设置为自定义布局方式，默认为空。如果为空，则通过itemPositioning属性来设置。
    /// 如果不为空则忽略itemPositioning,所以当tabBar的itemCustomPositioning属性不为空时，如果想改变布局规则，请设置此属性而非itemPositioning。
    public var itemCustomPositioning: SPTabBarItemPositioning? {
        didSet {
    ...
    internal var containers = [SPTabBarItemContainer]()
    
    /// 缓存当前tabBarController用来判断是否存在"More"Tab
    internal weak var tabBarController: UITabBarController?
    
    /// 自定义'More'按钮样式，
    open var moreContentView: SPTabBarItemContentView? = SPTabBarItemMoreContentView() {
        didSet { self.reload() }
    }
    ...
    open override func beginCustomizingItems(_ items: [UITabBarItem]) {
        SPTabBarController.printError("beginCustomizingItems(_:) is unsupported in SPTabBar.")
        super.beginCustomizingItems(items)
    }
    
    open override func endCustomizing(animated: Bool) -> Bool {
        SPTabBarController.printError("endCustomizing(_:) is unsupported in SPTabBar.")
...
// MARK: - Layout
internal extension SPTabBar {
    
    func updateLayout() {
        guard let tabBarItems = self.items else {
            SPTabBarController.printError("empty items")
            return
        }
...
internal extension SPTabBar {
    ...
    func isMoreItem(_ index: Int) -> Bool {
        return SPTabBarController.isShowingMore(tabBarController) && (index == (items?.count ?? 0) - 1)
    }
    ...
    func reload() {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            let container = SPTabBarItemContainer.init(self, tag: 1000 + idx)
            self.addSubview(container)
            self.containers.append(container)
            
            if let item = item as? SPTabBarItem {
    ...
    @objc func highlightAction(_ sender: AnyObject?) {
        guard let container = sender as? SPTabBarItemContainer else {
            return
        }
    ...
    @objc func select(itemAtIndex idx: Int, animated: Bool) {
        ...
        if currentIndex != newIndex {
            if currentIndex != -1 && currentIndex < items?.count ?? 0{
                if let currentItem = items?[currentIndex] as? SPTabBarItem {
        ...
        for (idx, item) in tabBarItems.enumerated() {
            ...
            if let explicitLabel = item.accessibilityLabel {
                ...
                if self.isMoreItem(idx) {
                    accessibilityTitle = NSLocalizedString("More_TabBarItem", bundle: Bundle(for:SPTabBarController.self), comment: "")
                }
                ...
                let formatString = NSLocalizedString(item == selectedItem ? "TabBarItem_Selected_AccessibilityLabel" : "TabBarItem_AccessibilityLabel",
                                                     bundle: Bundle(for: SPTabBarController.self),
                                                     comment: "")
